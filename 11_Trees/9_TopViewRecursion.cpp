...
// Recursive function to fill top view
void Tview(Node* root, int pos, vector<int> &ans, vector<int> &level, int depth) {
    if (!root) return;

    // If first time visiting this horizontal distance or at smaller depth
    if (depth < level[pos]) {
        level[pos] = depth;
        ans[pos] = root->data;
    }
